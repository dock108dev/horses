...
at, and extends it with three new findings (F32–F34) for code surfaces
introduced in the API/data-loading recovery branch.
...
6. **E1 (Escalation) — UI tags / odds overrides are local-only.**
...
| F2  | `api/sources/twinspires.py:469`                      | teardown silence          | Note     | justify     |
| F3  | `api/main.py:330,351,488,514`                        | cache-fallback contract   | Low      | justify     |
| F4  | `api/refresh.py:54`                                  | per-leg downgrade         | Low      | justify     |
| F5  | `api/sources/twinspires.py:127`                      | odds parse fallback       | Note     | justify     |
| F6  | `api/cache.py:173`                                   | rollback + re-raise       | Note     | justify     |
| F7  | (reserved)                                           | —                         | —        | —           |
| F8  | `api/main.py:628,675`                                | sim/tickets envelope      | Low      | justify     |
| F9  | `api/sources/pick5.py:162`                           | tier-2 verification       | Note     | justify     |
| F10 | (removed — KentuckyDerby adapter deleted by SSOT pass) | —                       | —        | —           |
| F11 | `api/normalize.py:74,84`                             | odds parse → None         | Note     | justify     |
| F12 | `api/sources/pick5.py:102,117`                       | data-race attr parse      | Note     | justify     |
| F13 | `api/sources/twinspires.py:318`                      | odds-floor in `finally`   | Note     | justify     |
| F14 | `api/sources/equibase.py:216`                        | rate-limit in `finally`   | Note     | justify     |
| F15 | `api/sources/twinspires.py:483`                      | optional dependency       | Note     | justify     |
| F16 | `api/tickets.py:689`                                 | sim fallback              | Low      | justify     |
| F17 | `api/main.py:559,573` + `web/lib/api.ts:43`          | extras=forbid contract    | Note     | justify     |
| F18 | (reserved)                                           | —                         | —        | —           |
| F19 | (reserved)                                           | —                         | —        | —           |
| F20 | (reserved)                                           | —                         | —        | —           |
| F21 | (reserved)                                           | —                         | —        | —           |
| F22 | `api/model.py:469,477`                               | priors range key parse    | Note     | justify     |
| F23 | `api/model.py:718`                                   | NaN/inf guard             | Note     | justify     |
| F24 | `api/tickets.py:632`                                 | confidence neutral fallback | Note   | justify     |
| F25 | `api/sources/equibase.py:158`                        | teardown silence          | Medium   | **tightened** |
| F26 | `api/sources/twinspires.py:507`                      | curl_cffi inner close     | Note     | justify     |
| F27 | `api/cache.py:125`                                   | sqlite close silence      | Medium   | **tightened** |
...
| F30 | `web/app/sequence/[day]/page.tsx:108–225`            | catch-to-state            | Low      | justify     |
| F31 | `api/main.py:96` (`_redact_exc`)                     | error message redaction   | Note     | justify     |
| F32 | `api/sources/fixture.py:130–146`                     | fixture row silent-skip   | Note     | justify     |
| F33 | `api/sources/twinspires.py:427`                      | TwinSpires 404 → None     | Note     | justify     |
...
`api/sources/twinspires.py:469` (`_safe_close`). Close failures are
...
`api/main.py:330` (fixture card), `:351` (live card), `:488` (fixture
odds), `:514` (live odds). Each is `except Exception as exc:` followed
...
`api/refresh.py:54`. A TwinSpires hiccup on a single race is logged at
...
`api/sources/twinspires.py:127–133`. `Fraction(s).limit_denominator(50)`
...
`api/cache.py:173`. The `executemany` insert is wrapped in `BEGIN /
...
`api/main.py:628` (simulate), `:675` (tickets). Same `_log.exception`
...
`api/sources/pick5.py:162`. `_scrape_first_leg` failures are logged at
...
`api/normalize.py:74–80`, `:84–88`. `_parse_odds_to_decimal` returns
...
`api/sources/pick5.py:102–106`, `:117–120`. **Justified**: scraping a
...
`api/sources/twinspires.py:318–324`. The 30-second per-race odds poll
...
`api/sources/equibase.py:216–223`. Same pattern as F13 for the
...
`api/sources/twinspires.py:483–488`. `ImportError` on `curl_cffi`
...
when a 403 *does* arrive without a fallback configured.

### F16 — Sim engine fallback in tickets builder

`api/tickets.py:689–703`. `try/except` wraps the `sim.simulate` call
...
`api/main.py:559,573` + `web/lib/api.ts:43–47`. `SimulateRequest` and
...
`api/model.py:469,477`. `_odds_rank_multiplier` skips malformed range
...
`api/model.py:718–728`. `_compute_velocity` rejects non-finite
...
`api/tickets.py:632`. When the edge model has not run on a card,
...
`api/sources/equibase.py:158–168`. Previously
...
`api/sources/twinspires.py:507–510`. The inner `_CurlCffiClient.close`
...
`api/cache.py:125–133`. Previously `self._conn.close()` without
...
`web/app/sequence/[day]/page.tsx:108–230`. Every async action
...
`api/main.py:96–108`. Strips `https?://...` and `/path/segments/...`
...
`api/sources/twinspires.py:425–430`. After a 403 fallback retry,
...
F1, F2, F5, F6, F9–F15, F17, F22–F24, F26, F28–F34. Every one of
...
F3, F4, F8, F16, F32, F33, F34. F3/F4/F8/F16 had inline references to
"finding F3 / F4 / F8 / F16" that pointed at the deleted
`docs/audits/` reports. This file re-establishes those targets so the
inline comments resolve. F32 and F33 are new this pass and have
inline comments added pointing here. F34's rationale lives in the
`web/lib/format.ts` module docstring (the cleanup pass folded the
per-component duplicates into that single shared file); no inline
`F34` cite is present at the call site, but the docstring above the
formatters captures the same NaN/Infinity/null contract.
...
F25, F27. Both fixed in a prior pass and verified in the current code.
Re-running `pytest api/tests` shows 373 passed, 0 failed.
...
- `pytest api/tests` — 373 passed, 0 failed (post-tightening, post-F32
  and F33 inline-comment additions).
- No broad-catch behavior was changed in this pass; inline comments
  were added at the F32 and F33 sites and line-number drift was
  reconciled in the findings table. F34 was reclassified to
  `web/lib/format.ts` after the cleanup pass moved the formatters there.
